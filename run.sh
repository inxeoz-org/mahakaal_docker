...
if [ $# -lt 2 ]; then
  echo "Usage: $0 <apps_json_file> <apps_to_install> [port] [--no-cache]"
  echo "Example: $0 apps_mahakaal.json 'mahakaal' 8890 --no-cache"
  exit 1
fi
...
docker compose -f pwd.yml build $NO_CACHE
